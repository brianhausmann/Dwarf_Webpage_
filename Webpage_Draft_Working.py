...
with open('webpage_draft2.html','w') as f:
    ...
    dtype_dict = collections.OrderedDict([(0, [('objid', '<i8'), ('type', 'S6'), ('name', 'S12'), ('ra', '<f8'), ('dec', '<f8'), ('u', '<f8'), ('g', '<f8'), ('r', '<f8'), ('i', '<f8'), ('z', '<f8'), ('redshift', '<f8')]),(1,[('name','S12'),('objID','<i8'),('petroMag_r','<f8'),('petroR50_r','<f8'),('type','S8')]),(2,[('name','S12'),('zErr','<i10'),('zWarning','<i8'),('class','S10')])])
    ...
    for word in cat_list:
        ...
        for i in range(len(new_data['ra'])):
            if i%5 ==0:
                f.write('</tr><tr>')
            petro = new_data['petromag_r'][i] / 25
